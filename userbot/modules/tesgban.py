...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("`Terjadi Kesalahan... Mohon Lapor Ke Grup` @hmmm", str(err))
...
async def gben(userbot):
    ...
    await dark.edit(f"`Global Banned Akan Segera Aktif, Anda Akan Dibanned Secara Global Oleh Lu ヅ`")
    ...
    if user:
        if user.id == 870471128:
            return await dark.edit(
                f"`Anda Tidak Bisa Melakukan Global Banned, Dia Adalah Pembuat Saya ヅ`"
            )
        try:
...
async def gunben(userbot):
    ...
    try:
        ...
        return await dark.edit("`Terjadi Kesalahanヅ`")
    if user:
        if user.id == 1353102497:
            return await dark.edit("**Gabisa gban ini owner botヅ**")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"`cancelled Global Banned... Memproses... `")
            except BaseException:
                b += 1
    else:
        await dark.edit("`balas pesan target tololヅ`")
    try:
